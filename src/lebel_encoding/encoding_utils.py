...
from lebel_encoding.ridge_utils.npp import mcorr, zscore
from lebel_encoding.ridge_utils.utils import make_delayed
...
def run_permutation_test(zPresp, pred, blocklen, nperms, mode="", thres=0.001):
    ...
    for start in range(0, nvox, partlen):
        ...
        pv, pr, rs = permutation_test(
            zPresp[:, start : start + partlen],
            pred[:, start : start + partlen],
            blocklen,
            nperms,
        )
        pvals.append(pv)
    ...
    cci.upload_raw_array(
        os.path.join(save_location, "%sgood_voxels" % mode), (pvals <= pN)
    )
    cci.upload_raw_array(
        os.path.join(save_location, "%spN_thres" % mode),
        np.array([pN, thres], dtype=np.float32),
    )
    return
