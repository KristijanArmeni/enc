...
from lebel_encoding.ridge_utils.interpdata import lanczosinterp2D
from lebel_encoding.ridge_utils.SemanticModel import SemanticModel
from lebel_encoding.ridge_utils.stimulus_utils import (
    load_simulated_trfiles,
    load_textgrids,
)
...
def get_phonemerate_vectors(allstories):
    ...
    Returns:
            Dictionary of {story: downsampled vectors}
    """
    with open(join(DATA_DIR, "lebel_data", "articulationdict.json"), "r") as f:
        artdict = json.load(f)
    phonseqs = get_story_phonseqs(allstories)  # (phonemes, phoneme_times, tr_times)
    downsampled_arthistseqs = {}
    for story in allstories:
...
def get_eng1000_vectors(allstories):
    ...
    Returns:
            Dictionary of {story: downsampled vectors}
    """
    eng1000 = SemanticModel.load(join(DATA_DIR, "lebel_data", "english1000sm.hf5"))
    wordseqs = get_story_wordseqs(allstories)
    vectors = {}
    for story in allstories:
