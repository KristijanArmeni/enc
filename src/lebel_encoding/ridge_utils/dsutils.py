...
from lebel_encoding.ridge_utils.DataSequence import DataSequence
...
def make_character_ds(grids, trfiles):
    ...
    for st in stories:
        ...
        fixed_grtranscript = [
            (s, e, map(int, c.split(","))) for s, e, c in grtranscript if c
        ]
...
def make_character_model(dss):
    ...
    stories = dss.keys()
    storychars = dict(
        [(st, np.unique(np.hstack(ds.data))) for st, ds in dss.iteritems()]
    )
    ...
    for st in stories:
        char_inds[st] = dict(
            zip(storychars[st], range(ncharsdone, ncharsdone + len(storychars[st])))
        )
    ...
    for st, ds in dss.iteritems():
        ...
        charmodels[st] = DataSequence(
            charmat, ds.split_inds, ds.data_times, ds.tr_times
        )
...
def make_dialogue_model(ds):
    return DataSequence(
        np.ones((len(ds.data), 1)), ds.split_inds, ds.data_times, ds.tr_times
    )
    ...
    return dict(
        [
            (
                k,
                DataSequence(
                    np.hstack([s[k].data for s in seqs]), seqs[0][k].split_inds
                ),
            )
            for k in keys
        ]
